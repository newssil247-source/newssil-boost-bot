// NewsSIL Boost Bot — v4.2b (Railway Pro, full)
...
const WM_POS       = process.env.WM_POS || 'top-right'; // תמיד ימין; אפשר bottom-right אם תרצה
const WM_MARGIN    = Number(process.env.WM_MARGIN || 20);
const WM_WIDTH_PCT = Number(process.env.WM_WIDTH_PCT || 18);

// החלפה: מוחק יעד קודם ומעלה חדש (להגדלת נראות)
...
// SEO/GA/Booster/Meta
const SEO_ENABLE          = String(process.env.SEO_ENABLE || 'true') === 'true';
...
const META_COMPLIANCE_ENABLE = String(process.env.META_COMPLIANCE_ENABLE || 'true') === 'true';
const META_HARD = (process.env.META_BLOCK_TERMS_HARD || 'עירום,פורנו,גזענות,נאצים,טרור,ISIS,פצצה,אונס')
                    .split(',').map(s=>s.trim()).filter(Boolean);
const META_SOFT = (process.env.META_BLOCK_TERMS_SOFT || 'דם,גופות,קטין,ירי,אלימות קשה,שיסוי,הסתה')
                    .split(',').map(s=>s.trim()).filter(Boolean);

// Fan-out ל-Make (אותו webhook לכל הרשתות אם תרצה)
const X_WEBHOOK_URL  = process.env.X_WEBHOOK_URL  || process.env.MAKE_WEBHOOK_URL || '';
const FB_WEBHOOK_URL = process.env.FB_WEBHOOK_URL || process.env.MAKE_WEBHOOK_URL || '';
const IG_WEBHOOK_URL = process.env.IG_WEBHOOK_URL || process.env.MAKE_WEBHOOK_URL || '';
const TT_WEBHOOK_URL = process.env.TT_WEBHOOK_URL || process.env.MAKE_WEBHOOK_URL || '';
...
const SOCIAL_WM = {
  ...
  tt: true // TikTok תמיד מדבקה – לפי הדרישה שלך
};

// קבצים ל-state
const DATA_DIR  = 'data';
const SEEN_FILE = path.join(DATA_DIR, 'seen.json');  // אנטי-כפילות
const MAP_FILE  = path.join(DATA_DIR, 'map.json');   // מיפוי מקור→יעד למחיקה/החלפה
...
async function deleteOldIfExists(key, tg) {
  ...
  const oldId = map[key];
  if (!oldId) return;
  try { await tg.deleteMessage(TARGET_CHANNEL_ID, oldId); } catch {}
}
...
function addFooterToText(text) {
  ...
  return ft ? (clean ? `${clean}\n\n${ft}` : ft) : clean;
}

// Booster לחיפוש (תגיות)
...
// Meta compliance (למניעת חסימות בפייס/אינסטה)
...
async function seoIngest({ text, mediaType, sourceChat, msgId, tags=[] }) {
  ...
  try {
    await fetch(SEO_POST_ENDPOINT, {
      ...
      headers: { 'content-type':'application/json' },
      body: JSON.stringify({
        ts: Date.now(), mediaType, text, tags, sourceChat, msgId,
...
// Social footer (לטקסט שיוצא ל-Make)
...
async function fanoutToSocials({ text, mediaType, fileId, tg }) {
  ...
  const wantWM = SOCIAL_WM; // TikTok=true לפי הדרישה
...
if (DASHBOARD_ENABLE) {
  ...
  app.get('/api/stats', async (req,res)=>{
    ...
    if (DASHBOARD_AUTH_TOKEN && auth !== DASHBOARD_AUTH_TOKEN)
