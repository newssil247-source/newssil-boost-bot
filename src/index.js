// NewsSIL Boost Bot — v4.2b (Railway Pro, fixed)
...
const WM_IMAGE     = path.resolve(process.env.WM_IMAGE || 'assets/il_logo.png');
...
const TG_KEYWORDS = (process.env.TG_KEYWORDS || 'חדשות,ישראל,דחוף,מבזק,תיעוד,צפו,il,newsil,חדשות ישראל il')
  .split(',').map(s=>s.trim()).filter(Boolean);

// Fan-out (Make) — אותו webhook לכל הרשתות
const MAKE_WEBHOOK_URL = process.env.MAKE_WEBHOOK_URL || '';
const X_WEBHOOK_URL  = process.env.X_WEBHOOK_URL  || MAKE_WEBHOOK_URL;
const FB_WEBHOOK_URL = process.env.FB_WEBHOOK_URL || MAKE_WEBHOOK_URL;
const IG_WEBHOOK_URL = process.env.IG_WEBHOOK_URL || MAKE_WEBHOOK_URL;
const TT_WEBHOOK_URL = process.env.TT_WEBHOOK_URL || MAKE_WEBHOOK_URL;
const SOCIAL_FOOTER_ENABLE = String(process.env.SOCIAL_FOOTER_ENABLE || 'true') === 'true';
const SOCIAL_FOOTER_SHORT_FOR_X = String(process.env.SOCIAL_FOOTER_SHORT_FOR_X || 'false') === 'true';
const SOCIAL_WM = { x:true, fb:true, ig:true, tt:true }; // TikTok תמיד מדבקה
...
async function deleteOldIfExists(key,tg){ if(!REPLACE_TARGET) return; const m=await readMap(); const id=m[key]; if(!id) return; try{ await tg.deleteMessage(TARGET_CHANNEL_ID,id); }catch{} }
async function saveMapping(key,newId){ const m=await readMap(); m[key]=newId; await writeMap(m); }

// Footer המדויק שביקשת (מקובע בקוד)
function buildFooterHTML(){
  if(!FOOTER_VISIBLE_TG) return '';
  // שורה 1 קבועה
  const header = 'חדשות ישראל IL - תעקבו אחרינו';
  if (FOOTER_LINKED) {
    return `${header}\n` +
      `<a href="${LINK_X}">X</a> | ` +
...
function boostKeywords(s){ if(!TG_BOOSTER_ENABLE||!TG_KEYWORDS.length) return s; const tags=TG_KEYWORDS.slice(0,8).map(k=>`#${k.replace(/\s+/g,'')}`).join(' '); return `${s}\n\n${tags}`; }
...
function posExpr(pos,m){ const xR=`W-w-${m}`, xL=`${m}`, yT=`${m}`, yB=`H-h-${m}`; switch(pos){case 'top-right':return`${xR}:${yT}`;case 'bottom-right':return`${xR}:${yB}`;case 'bottom-left':return`${xL}:${yB}`;default:return`${xL}:${yT}`;} }
...
function buildSocialFooter({short=false}){
  return short
    ? `Follow: X | Facebook | WhatsApp | Instagram | TikTok`
    : `חדשות ישראל IL - תעקבו אחרינו\n` +
...
async function fanoutToSocials({ text, mediaType, fileId, tg }){
  if (!X_WEBHOOK_URL && !FB_WEBHOOK_URL && !IG_WEBHOOK_URL && !TT_WEBHOOK_URL) return;
...
bot.on('channel_post', async (ctx)=>{
  ...
  console.log('channel_post received from', post.chat.id, 'msg', post.message_id, 'photo?', !!post.photo, 'video?', !!post.video);
...
// Launch (מוחקים webhook כדי לפעול ב-polling)
(async ()=>{
  try{
    await bot.telegram.deleteWebhook({ drop_pending_updates: true })
      .then(()=>console.log('Webhook deleted, using long polling'))
...
const DASHBOARD_AUTH_TOKEN = process.env.DASHBOARD_AUTH_TOKEN || 'NEWSIL-PRO-IL';
if (DASHBOARD_ENABLE){
  const app=express(); app.use(express.json());
  const needAuth=(req)=> (DASHBOARD_AUTH_TOKEN && (req.headers['x-auth-token']||req.query.token)!==DASHBOARD_AUTH_TOKEN);

  app.get('/api/status', (_req,res)=>res.json({ok:true,ts:Date.now()}));
  app.get('/api/stats', async (req,res)=>{
    if(needAuth(req)) return res.status(401).json({ok:false});
    try{ const seen=await readSeen(); const map=await readMap(); res.json({ok:true, seen_count:seen.length, map_count:Object.keys(map).length}); }
    catch(e){ res.status(500).json({ok:false, error:String(e)}); }
  });
  app.get('/dashboard', (req,res)=>{
    if(needAuth(req)) return res.status(401).send('Unauthorized');
